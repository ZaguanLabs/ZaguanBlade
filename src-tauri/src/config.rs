...
**semantic_anchor_search** - Find rationale, design-document links, and indexed semantic literals
...
pub fn save_api_config(path: &Path, cfg: &ApiConfig) -> Result<(), String> {
    ...
    if cfg_to_write.api_key.trim().is_empty() {
        crate::credential_store::delete_api_key();
    } else if crate::credential_store::store_api_key(&cfg_to_write.api_key).is_ok() {
        cfg_to_write.api_key.clear();
    }

    let json = serde_json::to_vec_pretty(&cfg_to_write).map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn remote_config_stores_server_user_id() {
        ...
        assert_eq!(cfg.user_id, "user_canonical");
    }
