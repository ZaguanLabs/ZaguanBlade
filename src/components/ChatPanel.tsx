...
const PendingResponseIndicator: React.FC<{ toolActivity?: ToolActivityState | null }> = ({ toolActivity }) => {
    ...
    return (
        <div className="px-4 py-3">
            <div role="status" aria-live="polite" className={`inline-flex max-w-full items-center gap-3 rounded-[calc(var(--panel-radius)*1.2)] border px-4 py-3 text-[11px] font-medium text-(--fg-secondary) shadow-(--shadow-lg) ${borderClass} ${panelClass}`}>
                <div className={`flex h-8 w-8 items-center justify-center rounded-[calc(var(--panel-radius)*0.75)] border ${iconWrapClass}`}>
                    <Loader2 aria-hidden="true" className={`h-4 w-4 animate-spin ${iconClass}`} />
                </div>
                ...
                <div aria-hidden="true" className="ml-1 flex items-center gap-1">
